...
import phase_a_teacher_exemplar_v1 as candidate
...
def main():
    ...
    result = {
        "probe": "phase_a_teacher_exemplar_v1",
        "scope": "Phase A only, seeds 7004-7006, Day7-10 sampled decisions",
        "intervention": "If Self lacks the Teacher-predicted investment class, inject at most one concrete market order copied from the nearest historical Teacher state of the same class.",
        "boundary": "Teacher imitation probe only; not yet terminal Battle evidence.",
        "rows": rows,
        "summary": {
    ...
    Path("phase_a_teacher_exemplar_result_v1.json").write_text(json.dumps(result, ensure_ascii=False, indent=2))
