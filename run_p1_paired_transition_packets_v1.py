...
def summarize_farm(farm,private):
    ...
    for row in farm.get("tiles",[]):
        for tile in row:
            ...
            kind=tile.get("kind")
            if kind=="PLANT":
                c=tile.get("crop"); plants[c]=plants.get(c,0)+1
            elif kind=="WEED":
                weeds+=1
            if "animal" in tile:
...
def snapshot(env):
    ...
    core={
        ...
        "self":summarize_farm(farms[0],s0.private),
        "opponent":summarize_farm(farms[1],s1.private),
...
def main():
    ...
    out={
        "schema":"p1-paired-transition-packets-v1",
    ...
    Path("p1_paired_transition_packets_v1.json").write_text(json.dumps(out,ensure_ascii=False,indent=2)+"\n",encoding="utf-8")
